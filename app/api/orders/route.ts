...
export async function POST(request: Request) {
  ...
  if (body.autoFromStock) {
    ...
    const { data: allProducts } = await supabase.from('of_products').select('id, unit_price, order_quantity').eq('is_active', 1).eq('auto_order', 1).order('sort_order');
    const { data: stockItems } = await supabase.from('of_stock_check_items').select('product_id, needs_order, suggested_quantity').eq('stock_check_id', latestCheck.id);
    ...
    const items = allProducts.map(p => {
      ...
      return { productId: p.id, quantity: (si?.needs_order && si?.suggested_quantity) ? si.suggested_quantity : 0, unitPrice: p.unit_price };
    });
